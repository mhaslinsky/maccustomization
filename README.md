...
- **`apply` does not restart AdGuard**, and exclusion changes need `adguard-cli restart` to take effect. A restart drops the proxy path out from under whatever is using it, and has twice knocked the `codex-lb` container offline mid-session. Restart when nothing is mid-request.
...
- **Docker containers cannot reach this proxy and do not need to.** In a container, `host.docker.internal` resolves to an IPv6 address while AdGuard binds IPv4 loopback only, so the connection fails as "Network is unreachable" on any port. Fixing that would require binding the proxy to a non-loopback address, exposing an open MITM proxy to the LAN. There is also no upside, since `api.openai.com` is already excluded and would be tunneled unmodified. Leave container egress on Docker's own NAT.
