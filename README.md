...
- **[Thaw](https://github.com/stonerl/Thaw)** (Ice fork): reactivated 2026-05-27 for the Thaw 2.0 beta after Bartender 6 proved unstable on Tahoe. It remains a manual target and drives the full menu bar appearance, including the glass background.
...
- **Why not native Spaces:** on macOS 26 every userspace API for moving another app's window to a native Space silently no-ops (`hs.spaces.moveWindowToSpace`, `SLSMoveWindowsToManagedSpace`, `CGSAddWindowsToSpaces`, and synthesized `Ctrl+arrow` at HID level). The calls return success and the window does not move. AeroSpace sidesteps this by parking inactive windows offscreen through the Accessibility API, so it needs no private symbols and no SIP change.
- **Dock-state layouts:** `scripts/dock-layout.mts [docked|undocked|auto] [--dry-run]` sweeps every open window into a workspace and floats it. `--dry-run` prints the plan without moving anything. Undocked puts Zen on 3, Ghostty on 2, everything else on 1. Docked leaves placement alone, because every monitor already shows its own workspace and nothing is hidden; sweeping there would pile every window onto the one monitor holding workspace 1. `auto` picks by monitor count. Bound to `alt-shift-d`, with `alt-shift-u` forcing the undocked layout.
- **Dock and undock trigger:** `hammerspoon/dock_layout.lua` runs the sweep when a display is attached or removed, gated on monitor count because `hs.screen.watcher` also fires on resolution and arrangement changes. `ctrl+alt+cmd+D` runs it by hand.
- Anything invoking the script must go through a login shell (`/bin/zsh -lc`). AeroSpace's `exec-and-forget` uses `/bin/bash -c` with a minimal PATH that has no node, and it discards stdout, stderr and the exit code, so the script notifies on failure itself.
- The app-to-workspace map is `UNDOCKED_LAYOUT` at the top of `dock-layout.mts`. `DOCKED_LAYOUT` is `null`, meaning leave placement alone.
- New windows briefly size to the full workspace before flipping to floating. That is inherent to `on-window-detected` + `layout floating`, which is upstream's own answer for disabling auto-tiling; the window keeps whatever size you give it afterwards.
- Config is symlinked so this repo stays the source of truth: `ln -sf "/Users/mhaslinsky/Developer/mac-customization/aerospace/aerospace.toml" ~/.aerospace.toml`

**Network filtering** (`scripts/adguard-proxy.mts`): provisioning for the [AdGuard CLI](https://github.com/AdguardTeam/AdGuardCLI) HTTPS-filtering proxy, which runs on `127.0.0.1:3129` and is wired into the Wi-Fi service as the system HTTP and HTTPS proxy.

- **`npm run adguard:check`** reports drift and exits 1; **`npm run adguard:apply`** reconciles it. Check is the default, so a bare `./scripts/adguard-proxy.mts` only reads. `apply` takes `--dry-run`, and both take `--service <name>` for a service other than Wi-Fi.
- **It is not chained into `npm run build`.** Every other codegen here writes a config file. This one changes live system network settings, so it stays a deliberate invocation.
- **What it owns:** the HTTPS exclusion list (the MITM bypass), the proxy host, port, and bypass ranges, and a check that the four expected filter lists are enabled. That is roughly 20 lines of actual decision.
- **What it deliberately never touches:** the CA store under `SSL/`, the license, and the three filter databases. Those are signing-key material, a credential, and about 83 MB of regenerable vendor data. None of it belongs in a repo, so this is a provisioning script rather than a checked-in copy of `~/Library/Application Support/adguard-cli/`.
- **Exclusions live in a managed block** delimited by `# >>> mac-customization managed exclusions >>>`. Everything above it is AdGuard's shipped list, left untouched. A domain excluded here is one whose client pins certificates or ships its own CA bundle, so interception breaks it outright rather than degrading it. Add new ones to `EXCLUSION_GROUPS` in the script, never to the file by hand.
- **`apply` does not restart AdGuard**, and exclusion changes take effect only on restart. Restart with `launchctl kickstart -k gui/$(id -u)/com.adguard.cli`, never with `adguard-cli restart`: against the launchd-managed instance the CLI prints "The AdGuard proxy server is not running" and exits 0 while the proxy is serving traffic, so it restarts nothing and reports success (measured 2026-09-10 on v1.4.13; `adguard-cli status` lies the same way). A restart drops the proxy path out from under whatever is using it, and has twice knocked the `codex-lb` container offline mid-session. Restart when nothing is mid-request.
- **A blocked request reads as `HTTP 500` with an empty `text/plain` body**, delivered inside the intercepted TLS session. It is the block, not an error. Scriptlet targets differ again: `google-analytics.com/analytics.js` returns 200 with AdGuard's `noopFunc` stub, so check the body rather than the status. Prove filtering with a control: through the proxy `example.com` returns 200 while `doubleclick.net` returns 500, and directly `doubleclick.net` returns 301.
- **A container cannot dial this proxy, but its HTTPS goes through it anyway.** In a container, `host.docker.internal` resolves to an IPv6 address while AdGuard binds IPv4 loopback only, so an explicit connection fails as "Network is unreachable" on any port. Binding the proxy elsewhere would expose an open MITM proxy to the LAN, so leave container egress on Docker's own NAT. That NAT is not a bypass: Docker Desktop routes container ports 80 and 443 through the macOS system proxy, which is this one, so the exclusion list governs container traffic and an unexcluded host arrives re-signed by the AdGuard CA. Measured 2026-09-10, with `auth.openai.com` absent from the list: every `codex-lb` token refresh failed to verify, both accounts fell out of the pool, and every Codex pane on the machine got `No available accounts`.
...
- **`src/themes/<theme>.ts`**: per-theme design tokens, including `accents` (per-widget colors), `layout` (typography, spacing, blur, and shadow), `status` (good, warn, and bad colors), `primary` (cross-program accent for JankyBorders window borders and Bartender's menu bar border), `menuBarTint` (accent for Bartender's gradient edge, Warp's terminal accent and cursor, and Thaw's tint and gradient; kept distinct from translucent `primary.active`, which may be too faint on the menu bar), and `icons` (optional per-widget glyphs, typically Nerd Font codepoints with a Nerd Font fallback in `layout.fontStack`).
