...
- **Spotify auto-update wipes the patch; `scripts/spicetify-heal.mts` repairs it.** An update replaces `Apps/` with a stock `xpui.spa`. The right command is `spicetify backup apply`, which clears the stale backup itself when Apps is stock. Do NOT run `spicetify restore` first: it copies the OLD version's backup over the new install. `restore backup apply` is only for a patched Spotify whose `[Backup] with` differs from the installed CLI (after `spicetify upgrade`). `npm run spicetify:check` reports the state; the LaunchAgent `com.mhaslinsky.spicetify-heal` (installed by `npm run spicetify:install-agent`, from the primary checkout only) runs `heal` on any Spotify.app change and every 5 minutes. Heal waits out a 90s settle window and any active playback, upgrades the CLI, applies, then verifies every configured extension is referenced in `Apps/xpui/index.html`. Log: `~/Library/Logs/spicetify-heal.log`.
- **Marketplace installs are lost when Spotify rebuilds its browser cache.** The ledger lives in `~/Library/Caches/com.spotify.client/Browser/IndexedDB`. That folder was recreated on 2026-08-12 and 2026-08-27 but survived the 2026-09-07 update to 1.3.0, so it is not every update. Chromium stores IndexedDB keys as UTF-16: an ASCII grep for `marketplace:installed` finds nothing even when the theme is installed, so search for `catppuccin/spicetify/` (or the item's repo path) instead. Items that must persist belong on disk under `~/.config/spicetify/`; `check` warns while `current_theme = marketplace`.
