...
- **Anything Marketplace installs is lost on the same updates.** Its ledger lives in `~/Library/Caches/com.spotify.client/Browser/IndexedDB`, which Spotify recreates on update. Themes and extensions that should persist belong on disk under `~/.config/spicetify/`; `check` warns while `current_theme = marketplace`.
