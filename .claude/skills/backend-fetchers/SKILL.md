...
- **`status_fetch.py`**: aggregated dev-tool status. Claude / OpenAI / Kimi / MiniMax / GitHub / Linear via statuspage.io `/api/v2/summary.json`; DeepSeek via its Flashcat page's `summary/active` API (open incidents, worst component status wins); Grok via the status.x.ai RSS feed (incidents not yet RESOLVED); OpenRouter via its Datadog status page's `config.json` (worst component status wins); Meta AI is described in the `status_fetch.py` docstring; Gemini via filtered Google Cloud `incidents.json` (ongoing incidents whose `affected_products` include "Gemini", severity mapped from `status_impact`). Each provider wrapped in `safe_provider` so a single upstream failure can't take out the others. Each provider block also emits a `url` (the public dashboard) so the widget can render the description as a click-through link when the provider isn't operational. No config required.
