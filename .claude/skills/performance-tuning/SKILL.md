...
|---|---|---|
| Status | `120000` (2 min) | **Stays at 2 min**: user wants early warning on AI provider, GitHub and Linear outages. Do not bump during perf sweeps. The HTTP calls in `status_fetch.py` already run in parallel on a `ThreadPoolExecutor`, so a run costs roughly the slowest single provider. |
