...
- **`npm run build`** — runs `build:widgets` → `validate:controls` → `build:hammerspoon` → `build:borders` → `build:bartender` → `build:warp` → `build:obsidian` → `build:spicetify` in sequence. (`build:thaw` and `build:slack-css` are deprecated — not chained, still manually invokable; `build:slack` was always standalone.)
